...
export const createNewGame = (data: Omit<GameData, "id" | "rating" | "reviews">) => {
    return async (dispatch: AppDispatch) => {
        try {
            if (!data.name || !data.release_year || data.genre.length === 0 || !data.image || !data.description) {
                dispatch(setError("Por favor complete todos los campos obligatorios*"))
                return;
            };
            if (data.description.length < 300) {
                dispatch(setError("La descripción debe tener al menos 300 caracteres"));
                return;
            }
...
export const updateOneGame = (data: Omit<GameData, "id" | "rating" | "reviews">, id: string) => {
    return async (dispatch: AppDispatch) => {
        try {
            dispatch(setLoading(true));
            dispatch(setError(null));
