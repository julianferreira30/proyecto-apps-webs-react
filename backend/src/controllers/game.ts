...
export const getGameById = async (request: Request, response: Response, next: NextFunction) => {
    try {
        ...
        if (game) {
            ...
            return response.status(404).json({error: "No se pudo encontrar el juego con ese id"}).end();
        }
    } catch (error) {
        next(error);
    }
...
export const addGame = async (request: Request, response: Response, next: NextFunction) => {
    try {
        const user = await User.findById(request.userId);
        if (!user) {
            return response.status(401).json({ error: "Solo un usuario autenticado puede agregar un juego" });
        }

        const body = request.body;
        const creator = body.creator ? (validateInputString(body.creator, 1, 100) ? body.creator.trim() : "") : undefined;
        if (!validateInputString(body.name, 1, 100) || 
...
export const setGame = async (request: Request, response: Response, next: NextFunction) => {
    try {
        const user = await User.findById(request.userId);
        if (!user) {
            return response.status(401).json({ error: "Solo un usuario autenticado puede cambiar atributos de un juego" });
        }
        ...
        if (!user.added.includes(objectId)) {
            return response.status(401).json({ error: "Solo un usuario que haya agregado ese juego puede cambiar sus atributos" });
        }

        if (!(body.name || body.release_year || body.creator || body.genre || body.image || body.description)) {
            return response.status(400).json({ error: "Al menos un campo es requerido"});
        }
        if (body.rating || body.rating === 0 || body.reviews) {
            return response.status(400).json({ error: "No es posible cambiar estos campos"});
        }
        const name = body.name ? (validateInputString(body.name, 1, 100) ? body.name.trim() : "") : undefined;
        const release_year = body.release_year ? (validateInputNumber(body.release_year, 1972, new Date().getFullYear()) ? body.release_year : "") : undefined;
        const creator = body.creator ? (validateInputString(body.creator, 1, 100) ? body.creator.trim() : "") : undefined;
